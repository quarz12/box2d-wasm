...
b2ParticleSystem::~b2ParticleSystem() {
    ...
    FreeBuffer(&m_totalForceBuffer, m_internalAllocatedCapacity);
}
...
template <typename T>
void b2ParticleSystem::FreeUserOverridableBuffer(
...
// Reallocate a buffer
template <typename T>
T *b2ParticleSystem::ReallocateBuffer(
...
void b2ParticleSystem::ReallocateInternalAllocatedBuffers(int32 capacity) {
    ...
    if (m_internalAllocatedCapacity < capacity) {
        ...
        m_totalForceBuffer = ReallocateBuffer(
                m_totalForceBuffer, 0, m_internalAllocatedCapacity, capacity, false
        );
    ...
    class UpdateBodyContactsCallback : public b2FixtureParticleQueryCallback {
                ...
                b2Fixture *fixture, int32 childIndex, int32 particleIndex) override //check if particle hits fixture
        {
            ...
            if (d < m_system->m_particleDiameter && ShouldCollide(fixture, particleIndex)) {
                ...
                b2ParticleBodyContact& contact =fixture->IsSensor() ? m_system->m_sensorContactBuffer.Append() : m_system->m_bodyContactBuffer.Append();
...
void b2ParticleSystem::SolveCollision(const b2TimeStep &step) {
    ...
    class SolveCollisionCallback : public b2FixtureParticleQueryCallback {
        ...
        inline bool ShouldCollide(b2Fixture *const fixture,
                                  int32 particleIndex) {
            if (!fixture->HasCollision() || fixture->IsSensor())
                return false;
            if (m_contactFilter) {
                const uint32 *const flags = m_system->GetFlagsBuffer();
                if (flags[particleIndex] & b2_fixtureContactFilterParticle) {
...
void b2ParticleSystem::SolveTensile(const b2TimeStep &step) {
    ...
    for (int32 k = 0; k < m_contactBuffer.GetCount(); k++) {   //adjust direction?
        const b2ParticleContact &contact = m_contactBuffer[k];
...
void b2ParticleSystem::SolveAdhesion(const b2TimeStep &step) {
    for (int32 i = 0; i < m_count; i++) {
        ...
        b2AABB aabb;
        aabb.upperBound = pos + (m_def.adhesionRadius / 2);
        aabb.lowerBound = pos - (m_def.adhesionRadius / 2);
        ...
        public:
            ...
            explicit callback(b2GrowableBuffer<b2ParticleBodyContact> *contacts, int32 i, b2Vec2 *point) {
                m_contacts = contacts;
                index = i;
                m_point = point;
            };
            ...
            bool ReportFixture(b2Fixture *fixture) override {
                if (fixture->HasCollision() && !fixture->IsSensor()) {
                    ...
                    contact.index = index;
                    contact.fixture = fixture;
                    fixture->ComputeDistance(*m_point, &contact.weight, &contact.normal,
                                             fixture->GetShape()->GetType());
                    return true;
                } else return false;
            }
        } callback(&contacts, i, &pos);
        m_world->QueryAABB(&callback, aabb);
        if (contacts.GetCount() == 0)
            continue;
        b2ParticleBodyContact closest = contacts[0];
        for (int j = 1; j < contacts.GetCount(); ++j) {
            if (abs(contacts[j].weight) < closest.weight)
                closest = contacts[j];
        }
        float sign = closest.weight > 0 ? -1.0f : 1.0f;
        if (abs(closest.weight) > m_def.adhesionRadius / 2)
            continue;
        ParticleApplyForce(i, sign * closest.normal * ((m_def.adhesionRadius / 2) - abs(closest.weight)) *
...
void b2ParticleSystem::SolveFriction(const b2TimeStep &step) {
    ...
    for (int32 i = 0; i < m_contactBuffer.GetCount(); i++) {
        ...
        if (parallel || angle < 0.7 || angle > b2_pi - 0.7) {   //direction is parallel or somewhat close
            ...
            if (!(parallel1 || parallel2 || angleAToBVel < 0.9 || angleBToAVel < 0.9 || angleBToAVel > b2_pi - 0.9 ||
                  angleAToBVel > b2_pi - 0.9)) { //particles are not behind each other
...
template <typename T>
void b2ParticleSystem::SetUserOverridableBuffer(
...
void b2ParticleSystem::ApplyForce(int32 firstIndex, int32 lastIndex,
    ...
    if (IsSignificantForce(distributedForce)) {
        ...
        for (int32 i = firstIndex; i < lastIndex; i++) {
            m_totalForceBuffer[i] += distributedForce.Length();
